...
extern char	concatenate_argv[256];
void DOS_Shell::Run(void) {
	...
	VFILE_Register("snasm68k.exe",(Bit8u *)snasm68k,sizeof(snasm68k));
	...
	do {
		...
		} else 
		{
			...
			if(strlen(run_cmd.c_str())>0 && run_once==TRUE)
			{
				/*sprintf(input_line,"dir");
				ParseLine(input_line);	*/
				
				char* t=(char*)run_cmd.c_str();
				ParseLine(t);	
				run_once=FALSE;
				//exit=1;
			}
			else
			{
				...
				sprintf(input_line,"%c:",filename[0]);
				ParseLine(input_line);	
				
				sprintf(input_line,"snasm68k.exe %s",concatenate_argv);
				ParseLine(input_line);	
